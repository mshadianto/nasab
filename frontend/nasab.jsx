...
const FARAIDH={
  ...
  // Auto-detect heirs from family tree
  detectHeirs(pp,personId){
    const p=pp.find(x=>x.id===personId);if(!p)return{};
    ...
    return{husband:sp&&sp.gender==="male"?1:0,wife:sp&&sp.gender==="female"?1:0,sons:ch.filter(c=>c.gender==="male").length,daughters:ch.filter(c=>c.gender==="female").length,father:pa&&pa.gender==="male"?1:0,mother:pa&&pa.gender==="female"?1:0,grandfather:0,grandmother:0,brothers:sib.filter(s=>s.gender==="male").length,sisters:sib.filter(s=>s.gender==="female").length}
  }
};
...
const mFromAPI=m=>({id:m.id,name:m.name,gender:m.gender,birthDate:m.birth_date||'',deathDate:m.death_date||'',birthPlace:m.birth_place||'',notes:m.notes||'',parentId:m.parent_id||null,spouseId:m.spouse_id||null,location:m.location_lat?{lat:m.location_lat,lng:m.location_lng,address:m.location_address||''}:null,photo:m.photo||'',nik:m.nik||'',createdAt:m.created_at});
const mToAPI=m=>({name:m.name,gender:m.gender,birth_date:m.birthDate||'',death_date:m.deathDate||'',birth_place:m.birthPlace||'',notes:m.notes||'',parent_id:m.parentId||null,spouse_id:m.spouseId||null,location_lat:m.location?.lat||null,location_lng:m.location?.lng||null,location_address:m.location?.address||'',nik:m.nik||''});
const API={
...
function Dashboard({user,onSelectFamily,onLogout,onCreateFamily,onAdmin}){
  const [fams,setFams]=useState([]);const [show,setShow]=useState(false);const [nn,setNn]=useState("");const [nd,setNd]=useState("");
...
function PersonForm({person:p,pp,onSave,onClose}){const ie=!!p?.id;const[f,sF]=useState({name:p?.name||"",gender:p?.gender||"male",birthDate:p?.birthDate||"",deathDate:p?.deathDate||"",birthPlace:p?.birthPlace||"",notes:p?.notes||"",parentId:p?.parentId||"",spouseId:p?.spouseId||"",location:p?.location||null,nik:p?.nik||""});const set=(k,v)=>sF(x=>({...x,[k]:v}));
...
function FaraidhCalc({pp,onClose}){
  const[pid,setPid]=useState("");const[total,setTotal]=useState(1000000000);
  ...
  const autoDetect=useCallback((id)=>{if(!id){setH(defaultH);return}const d=FARAIDH.detectHeirs(pp,id);setH(d)},[pp]);
  useEffect(()=>{if(pid)autoDetect(pid)},[pid]);
  const results=useMemo(()=>FARAIDH.calc(h,total),[h,total]);
  ...
  return(<div className="modal-ov" onClick={onClose}><div className="modal" onClick={e=>e.stopPropagation()} style={{maxWidth:680}}><div className="m-hdr"><h2>⚖️ Kalkulator Faraidh</h2><button className="btn btn-icon btn-ghost" onClick={onClose}><Ic.X/></button></div><div className="m-body">
    <div style={{fontSize:10,color:"var(--t3)",marginBottom:12,lineHeight:1.6}}>Kalkulator waris Islam berdasarkan hukum faraidh. Pilih almarhum untuk auto-deteksi ahli waris dari silsilah, atau isi manual.</div>
    <div className="far-sel"><label style={{fontSize:11,fontWeight:600}}>Almarhum:</label><select className="fsel" value={pid} onChange={e=>{setPid(e.target.value)}} style={{flex:1,maxWidth:250}}><option value="">— Pilih (opsional) —</option>{deceased.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></div>
    ...
    <div style={{fontSize:10,fontWeight:600,color:"var(--t2)",marginBottom:6}}>Ahli Waris</div>
    ...
    {results.length>0&&<div className="far-res"><div style={{fontSize:10,fontWeight:600,color:"var(--t2)",marginBottom:6}}>Pembagian Waris {results[0]?.awl&&<span style={{color:"var(--rose)",fontSize:9}}>(Awl — proporsional karena bagian melebihi harta)</span>}</div>
      <table className="far-table"><thead><tr><th>Ahli Waris</th><th>Jumlah</th><th>Bagian</th><th>Hak</th><th>Per Orang</th></tr></thead><tbody>
        {results.map((r,i)=><tr key={i}><td>{r.heir} <span className={`far-type ${r.type}`}>{r.type}</span></td><td style={{fontFamily:"var(--f-mono)"}}>{r.count}</td><td style={{fontFamily:"var(--f-mono)"}}>{r.frac} ({(r.share*100).toFixed(1)}%)</td><td style={{fontFamily:"var(--f-mono)",fontWeight:600}}>{fmt(r.amount)}</td><td style={{fontFamily:"var(--f-mono)",color:"var(--t3)"}}>{r.count>1?fmt(r.amountPer):"-"}</td></tr>)}
        <tr style={{fontWeight:700,borderTop:"2px solid var(--bdr)"}}><td colSpan={3}>Total Terdistribusi</td><td style={{fontFamily:"var(--f-mono)",color:Math.abs(totalDist-total)<1?"var(--pri)":"var(--rose)"}}>{fmt(totalDist)}</td><td style={{fontFamily:"var(--f-mono)",color:"var(--t3)"}}>{(totalDist/total*100).toFixed(1)}%</td></tr>
      </tbody></table>
      {totalDist<total-1&&<div style={{fontSize:10,color:"var(--t3)",marginTop:8}}>Sisa: {fmt(total-totalDist)} — dikembalikan ke ahli waris asabah atau baitul mal</div>}
    </div>}
    {results.length===0&&<div style={{textAlign:"center",padding:20,color:"var(--t3)",fontSize:12}}>Tambahkan ahli waris untuk melihat perhitungan</div>}
    <div style={{marginTop:14,padding:"8px 10px",background:"var(--bg2)",borderRadius:6,fontSize:9,color:"var(--t3)",lineHeight:1.6,fontFamily:"var(--f-mono)"}}>⚖️ Disclaimer: Kalkulator ini menggunakan hukum faraidh dasar. Untuk kasus kompleks (wasiat, hutang, dll), konsultasikan dengan ahli waris/ulama.</div>
  </div></div></div>)}
